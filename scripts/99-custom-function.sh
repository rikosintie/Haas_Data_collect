...
zsh -c '
  ...
  # successfully. Previously hardcoded to haas's own copy specifically.
  source /home/haas/Haas_Data_collect/haas-aliases.zsh 2>/dev/null

  if typeset -f haas-help >/dev/null; then
        haas-help

  else
    echo "haas-help not found in /home/haas/Haas_Data_collect/haas-aliases.zsh"
  fi
'
