...
The first step is downloading the script from my [GitHub Repository](https://github.com/rikosintie/Haas_Data_collect). There are several ways to accomplish that task depending on your Operating System and how deep you want to go down the rabbit hole!
...
Finally, if you have `git` installed your PC you can simply clone the repository using:
...
## Configure the CNC controls to output dprnt to a PC
...
The port number doesn't have to be unique per machine. The IP address of the  CNC control is unique so the ports can overlap. I like to use unique port numbers. For example, my shop has six machines and I use:
...
When you execute the script, `Haas_logger2.py`, runs continuously until you press `ctrl+c`. When `DPRNT[End of Cycle]` is executed in the CNC program, the script writes the data to disk and then resumes listening.

The script accepts three parameters. Multiple copies of the script can be started on the same server to collect from multiple CNC machines concurrently.
